...
Annotates all GSM samples in a GEO series with a **single LLM call per series**.
Input is one structured Family SOFT series entry; output includes per-sample labels
plus series-level `disease_normalized`, `tissue_normalized`, and `reasoning`.
...
```python
from geo_agent.skills.multiomics_analyze_series import MultiomicsSeriesAnalyzerSkill

skill = MultiomicsSeriesAnalyzerSkill(llm_client=ollama_client)
